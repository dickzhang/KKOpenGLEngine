...
{
	auto it = std::find(m_loading.begin(),m_loading.end(),page);
	if(it==m_loading.end())
		//if (m_loading.find(page) == m_loading.end())
	{
		auto lruit = std::find(m_lru_used.begin(),m_lru_used.end(),page);
		if(lruit!=m_lru_used.end())
...
bool PageCache::request(Page request)
{
	...
	{
		auto lruit = std::find(m_lru_used.begin(),m_lru_used.end(),request);
		if(lruit==m_lru_used.end())
			//if(m_lru_used.find(request)==m_lru_used.end())
		{
			//m_loading.insert(request);
			m_loading.push_back(request);
...
void PageCache::clear()
{
	...
	{
		auto lruit = std::find(m_lru_used.begin(),m_lru_used.end(),lru_page.m_page);
		if(lruit!=m_lru_used.end())
